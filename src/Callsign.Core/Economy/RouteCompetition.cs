...
public static class RouteCompetition
{
    ...
    public sealed record Standing(double YourShareMilli, double LoadMultiplier, IReadOnlyList<Rival> Rivals);
    ...
    public static Standing Evaluate(EconomyConfig cfg, Guid routeId, int yourRepMilli, int yourFareMilli)
    {
        var rivals = Rivals(cfg, routeId);
        double you = Attractiveness(cfg, yourRepMilli, yourFareMilli);
        double rivalSum = rivals.Sum(r => Attractiveness(cfg, r.ReputationMilli, r.FareMultiplierMilli));
        double share = you / (you + rivalSum);
        ...
        return new Standing(share * 1000.0, mult, rivals);
    }
